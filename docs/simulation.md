...
Opt-in browser-based UI on `http://localhost:5800` — field view, virtual gamepad, a Driver Station panel with alliance/enable controls, an **Auto/Teleop mode** toggle, an **auto-routine selector**, the `DemoIntake` demo, a **Mechanisms 3D** isometric panel (bottom-right of the field; live wireframe of the chassis, the swerve wheels at their live steer angles, and every mechanism at its `visualPose3d` mount — drag to orbit, click the title to collapse it (which stops its poll/draw, handy on narrow screens); backed by `/api/mechanisms3d`; see [docs/mechanisms.md](mechanisms.md)), and a **gyro-heading indicator** on the robot (cyan ring + needle showing the raw gyro yaw — `gyroRad` in `/api/state` — independent of the pose-based body arrow, so a divergence under vision correction is visible), a held/scored counter panel in the top-right corner, and a **live LED strip display** under the field (mirrors the robot's `LedStripSegments` colours via the `"leds"` array in `/api/state`; hidden when the robot has no strip — see [docs/leds.md](leds.md)). `-PwebUI` skips `wpi.sim.addGui()` / `addDriverstation()`, so **no Glass window opens**; the browser is the only UI.
